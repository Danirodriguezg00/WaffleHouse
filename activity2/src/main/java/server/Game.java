...
public class Game {
    ...
    private int tempCol1; // temp variable to store the column

    private int tempRow1; // temp variable to store the row

    private int tempCol2; // temp variable to store the column

    private int tempRow2; // temp variable to store the row
    ...
    public synchronized boolean checkMatch(){
        if(getTile(tempRow1,tempCol1) == getTile(tempRow2,tempCol2)){
            checkWin();
            return true;
        }
        else{
    ...
    public synchronized void flipTiles(int tile1row, int tile1col, int tile2row, int tile2col){
        replaceOneCharacter(tile1row, tile1col);
        replaceOneCharacter(tile2row, tile2col);
    }

    public synchronized void setTempCol1(int c){
        tempCol1 = c;
    }

    public synchronized void setTempCol2(int c){
        tempCol2 = c;
    }

    public synchronized int getTempCol1(){
        return tempCol1;
    }

    public synchronized int getTempRow1(){
        return tempRow1;
    }

    public synchronized void setTempRow1(int r){
        tempRow1 = r;
    }

    public synchronized void setTempRow2(int r){
        tempRow2 = r;
    }

    public synchronized int colSize(){
        return col;
    }

    public synchronized boolean checkFlipped(int row, int col){
        if(hidden[row][col] == '?')
            return false;
        else
            return true;
